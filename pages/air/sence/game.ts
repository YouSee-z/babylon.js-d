...
export class Game {
  ...
  async createScene() {
    ...
    this._engine.runRenderLoop(() => {
      this.scene.render();
      document.getElementById("showFps").innerHTML = `fps:${this._engine
        .getFps()
        .toFixed()}`;
    });
  ...
  private renderDom() {
    ...
    for (let i = 0; i < this.flyList.length; i++) {
      ...
      b2.onclick = () => {
        this.disposeFly(list.className);
      };
      list.appendChild(b2);
      ...
      b3.onclick = () => {
        this.explodeFly(list.className);
      };
      list.appendChild(b3);
    }
  }
  ...
  private addFly() {
    ...
    this.flyList[_index] = new F18Physics(
      this._canvas,
      this._engine,
      this.scene,
      this.flyMesh
